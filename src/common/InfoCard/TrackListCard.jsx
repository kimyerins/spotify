...
const TrackListCard = ({item, index}) => {

    console.log('itemttmtmt', item)

    const formatTime=(ms)=>{
        const minutes = Math.floor(ms / 60000);
        const seconds = Math.floor((ms % 60000) / 1000);

        const formaatedSeconds = seconds < 10 ? `0${seconds}`:seconds;

        return `${minutes}:${formaatedSeconds}`;
    }
  ...
  return (
    <div className={'px-4 py-2 h-14 flex gap-x-4 overflow-hidden rounded-[4px] group hover:bg-[hsla(0,0%,100%,.1)] transition-all duration-200 ease-in-out'}>
        <div className={'flex items-center justify-center'}>
            <span className={'text-[#b3b3b3] font-bold'}>{index+1}</span>
        </div>
        <div className={'w-[70%] flex items-center justify-start'}>
            <img className={'overflow-hidden rounded-[4px] h-full mr-3'} loading="lazy" src={item.album.images[1].url} alt="" />
        ...
        <div className={'w-[10%] flex items-center justify-end'}>
            ...
            <span className={'mr-3 text-[#b3b3b3]'}>{formatTime(item.duration_ms)}</span>
            <div>
                <svg
                    data-encore-id="icon"
                    role="img"
